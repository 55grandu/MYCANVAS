        ...
        groupBolaAmarrilla.select("circle[id='bolaAmarilla']").animate({r:61.666},2000,mina.bounce, function(){
            ...
            groupBolaVerde.select("circle[id='bolaVerde']").animate({cx:197.713, cy:75.616, r:41.667},2000,mina.bounce, function(){
                ...
                circuloAzul.animate({strokeDashoffset:"0"},2030, function(){
                    ...
                    // Animamos la linea
                    rellenoCirculoBlanco.animate({strokeDashoffset:"0"},2030, function(){
                        // Esperamos dos segundos para cerrar la animacion
                        setTimeout(function(){
                            // Recogemos el semi circulo blanco
                            bordeCirculoBlanco.animate({strokeDashoffset:"665.4508056640625"},2000, function(){
                                // Recogemos el semi circulo azul
                                contornoCirculoAzul.animate({strokeDashoffset:"1414.460693359375"},2000, function(){
                                    // Minimizamos el grupo verde
                                    groupBolaVerde.select("circle[id='bolaVerde']").animate({cx:198.047, cy:220.616, r:0},2000, function(){
                                        // Minimizamos el grupo amarillo
                                        groupBolaAmarrilla.select("circle[id='bolaAmarilla']").animate({r:0},2000);
                                        groupBolaAmarrilla.select("path[id='rellenoBrilloBolaAmarilla']").animate({d:"M198.047,220.616"},2000);
                                        groupBolaAmarrilla.select("path[id='bordeBrilloBolaAmarilla']").animate({d:"M198.047,220.616"},2000);
                                    });
